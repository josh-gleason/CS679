%----------------------------------------------------------
%
%  Program: bhattacharyyaError
%
%  Purpose: gives the bhattahcaryya bound to probability of error
%
...
function bError = bhattacharyaaErrorNormal(classifierParams)
%    samples(iClass).classifierScore = bayesianClassifier(samples(iClass).features,classifierParams(iClass));
   ...
   bErrArg = beta*(1-beta)/2*(meanV1-meanV2)'*...
