...
accuracy = zeros(length(costrange), 4);

% Test the Polynomial Kernel
for degree = degrange
    for cidx = 1:length(costrange)
        cost = costrange(cidx);
        accuracy(cidx,1:3) = main('classifier', SVM_CLASSIFIER, 'experiment', EXP_SVM_TRAIN_TEST, ...
            'resolution', resolution, 'kernel', SVM_KERNEL_POLY, ...
            'gamma', gamma, 'coef0', coef0, 'degree', degree, 'cost', cost, 'nfeats', NUM_FEATS);         
        accuracy(cidx,4) = mean(accuracy(cidx,1:3));
    end
    [accmax, idxmax] = max(accuracy(:,4));
    ...
    bar(accuracy);
    ax = axis();
    axis([ax(1) ax(2) 45 100]);
    set(gca, 'XTickLabel', costrange)
    xlabel('Cost');
    ylabel('Accuracy');
